...
object FluidPhysicsConfig extends Config[FluidPhysicsConfig] {
  ...
  private def waterBiomes: Seq[(Identifier, Biome)] =
    BuiltinRegistries.BIOME.streamEntries.iterator.asScala
      .filter { entry =>
          val category = Biome.getCategory(entry)
  ...
  object FluidGroup {
    ...
    lazy val groups: Map[Fluid, FluidGroup] = {
      val fluids: Seq[Fluid] = Registry.FLUID.streamEntries.iterator.asScala.map(_.value).toSeq
      fluids
