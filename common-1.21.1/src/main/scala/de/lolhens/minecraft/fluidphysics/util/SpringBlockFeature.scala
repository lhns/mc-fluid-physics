...
object SpringBlockFeature {
  def generate(world: LevelAccessor,
               blockPos: BlockPos,
    ...
    FluidPhysicsMod.config.spring.map(_.getBlock) match {
      case Some(springBlock) =>
        ...
        def isValidBlock(direction: Direction): Boolean =
          springFeatureConfig.validBlocks.iterator.asScala.exists(validBlock =>
            validBlock.value == world.getBlockState(blockPos.relative(direction)).getBlock)
