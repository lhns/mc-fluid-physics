...
import net.minecraft.world.level.material.{Fluid, FluidState, FlowingFluid}
...
object FluidSourceFinder {
  ...
  def setOf(blockPos: java.util.Collection[BlockPos]): mutable.Set[BlockPos] =
    blockPos.asScala.to(mutable.Set)
  ...
  // Java-friendly overloads. Mixin-injected code runs in the classloader of the vanilla MC class
  // it patches, which on NeoForge cannot see `scala.Option` (scala-library lives in the plugin
  // layer, not the boot layer). Returning a nullable BlockPos keeps the injected bytecode free of
  // Scala runtime types.

  def findSourceOrNull(world: LevelAccessor,
                       blockPos: BlockPos,
                       fluid: Fluid): BlockPos =
    findSource(world, blockPos, fluid).orNull

  def findSourceOrNull(world: LevelAccessor,
                       blockPos: BlockPos,
                       fluid: Fluid,
                       direction: Direction,
                       ignoreBlocks: java.util.Collection[BlockPos],
                       ignoreFirst: Boolean,
                       ignoreLevel: Boolean): BlockPos =
    findSource(world, blockPos, fluid, direction, setOf(ignoreBlocks), ignoreFirst, ignoreLevel).orNull
  ...
  def moveSource(world: LevelAccessor,
    ...
    dstState.getBlock match {
      case liquidBlockContainer: LiquidBlockContainer =>
        // LiquidBlockContainer.placeLiquid replaces the older tryFillWithFluid
