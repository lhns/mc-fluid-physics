...
import net.minecraft.world.level.material.{Fluid, Fluids, FlowingFluid}
...
case class RemoveLayersCommand(source: CommandSourceStack,
  ...
  override def run(): Either[String, Unit] = {
    source.sendSuccess(() => Component.literal("Starting layer removal"), false)
  ...
  private def drainFluid(world: Level, pos: BlockPos, state: BlockState): Unit = state.getBlock match {
    case bucketPickup: BucketPickup if !state.getBlock.isInstanceOf[LiquidBlock] =>
      bucketPickup.pickupBlock(null, world, pos, state)
  ...
  override def tick(worldContext: Level): Unit = {
    ...
    chunkQueues.headOption match {
      ...
      case None =>
        setTicking(false)
        source.sendSuccess(() => Component.literal("Layer removal finished"), false)
    }
  }
  ...
  override def cancel(worldContext: Option[Level]): Unit = {
    ...
    setTicking(false)
    source.sendSuccess(() => Component.literal("Layer removal cancelled for this world"), false)
  }
}
...
object RemoveLayersCommand {
  def execute(context: CommandContext[CommandSourceStack]): Either[String, Unit] = {
    ...
    val world: ServerLevel = player.serverLevel()
    ...
    fluidOption
      ...
      .map { fluid =>
        ...
        context.getSource.sendSuccess(
          () => Component.literal(s"Removing $numLayers layers for ${BuiltInRegistries.FLUID.getKey(fluid)} at $pos\n$typeConfirm"),
          false
        )
        CommandHandler.addPending(player, RemoveLayersCommand(context.getSource, world, pos, fluid, numLayers))
      }
  }
}
